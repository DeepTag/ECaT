    ...
    def select_patch(self, feat, patch_ids=None):
        ...
        if self.patch_nums > 0:
            if patch_ids is None:
                patch_ids = torch.randperm(feat_reshape.size(1), device=feat.device)  # randperm(3) = [2, 0, 1]
...
class SpatialCorrelativeLoss(nn.Module):
    ...
    def compare_sim(self, sim_src, sim_tgt, sim_other):
        ...
        if self.loss_mode == 'info' or sim_other is not None:
            ...
            num = int(N / 4)
            src = torch.where(sim_tgt < tgt_sorted[:, :, num:num + 1], 0 * sim_src,
                              sim_src)  # ignore small similarity pixels
...
class Normalization(nn.Module):
    def __init__(self, device):
        ...
        # mean = torch.tensor([0.485]).to(device)
        # std = torch.tensor([0.229]).to(device)
